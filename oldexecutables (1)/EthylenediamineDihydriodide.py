...
#chemical 3 (Amine1)
chem3_abbreviation = 'EthylenediamineDihydriodide'          # Abbreviation from chemical list https://goo.gl/UZSCBj  #Ensure: https://goo.gl/UZSCBj present, Reagent 2 amine
...
reag2_target_conc_chemical2 = 3.08      # PbI2 target molarity [M] [range > 0] v1.1) in the final solution (not accounting for non-idea solvent behavior)
reag2_target_conc_chemical3 = 1.54      # Amine 1  target molarity [M]
...
reag3_target_conc_chemical3 = 6     # Amine 1  target molarity [M]
...
parser.add_argument('--cp', default=0, type=int, choices=[0,1], 
    help='Activates the challenge problem pipeline. Default = 0 (standard pipeline)') #Default, debugging on and real code off == "1"
parser.add_argument('-d', '--debug', default=0, type=int, choices=[0,1],
    help='Disables dataupload. Default = 0 (Upload enabled)') #Default, debugging on and real code off == "1"
